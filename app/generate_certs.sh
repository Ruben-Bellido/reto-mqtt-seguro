...
openssl req -new -x509 -days 1826 -key $DIR/ca.key -out $DIR/ca.crt -subj "$SUBJECT_CA" -addext "subjectAltName=IP:$IP"
...
openssl req -new -out $DIR/server.csr -key $DIR/server.key -subj "$SUBJECT_SERVER" -addext "subjectAltName=IP:$IP"
...
openssl x509 -req -in $DIR/server.csr -CA $DIR/ca.crt -CAkey $DIR/ca.key -CAcreateserial -out $DIR/server.crt -days 365 -copy_extensions copy
...
openssl req -new -out $DIR/client.csr -key $DIR/client.key -subj "$SUBJECT_CLIENT" -addext "subjectAltName=IP:$IP"
# Usa la CSR y la CA para firmar la solicitud del cliente mqtt y obtener el certificado válido durante un año (sha256WithRSAEncryption)
openssl x509 -req -in $DIR/client.csr -CA $DIR/ca.crt -CAkey $DIR/ca.key -CAcreateserial -out $DIR/client.crt -days 365 -copy_extensions copy
